...
if (compare_trajectories){
  ...
  lines_df = data.frame(t = 1:N, sgd_t=sgd_t, rootsgd_t=rootsgd_t, ntsgd_t=ntsgd_t, tsgd_t=tsgd_t,
                        asgd_t=asgd_t, aisgd_t=aisgd_t, isgd_t=isgd_t,
                        target=true_theta) %>%
                                ...
                                ifelse(Method=="ntsgd_t", "Noisy truncated SGD",
                                       ifelse(Method=="tsgd_t", "Truncated SGD",
                                              ifelse(Method=="asgd_t", "Averaged SGD",
                                                     ifelse(Method=="aisgd_t", "Averaged implicit SGD",
                                                            ifelse(Method=="isgd_t", "Implicit SGD",                                                          ifelse(Method=="target", "(target)", "EXTRA"))))))))) %>%
    
    mutate(Method = factor(Method, levels = c("Vanilla SGD", "Averaged SGD", "Implicit SGD", "Averaged implicit SGD", "Root SGD", "Truncated SGD", "Noisy truncated SGD", "(target)"))) 
  ...
  if (paper_settings_convergence1){
    p1 = ggplot(lines_df, aes(x=t, y=theta_t, color=Method, linewidth = Method)) +
      ...
      scale_linewidth_manual(values = c("(target)" = big_line, "Vanilla SGD" = small_line, "Root SGD" = small_line, "Noisy truncated SGD" = small_line, "Truncated SGD" = small_line, "Averaged SGD" = small_line, "Averaged implicit SGD"=small_line, "Implicit SGD"=small_line)) +
  ...
  if (save_it){
    setwd(your_wd_plots)
    ggsave(paste0("plots/Cleaned_R/other_methods_examples_D",D, ".png"), width = 25, height = 15, units = "cm")
