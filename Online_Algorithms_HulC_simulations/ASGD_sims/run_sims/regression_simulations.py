...
This file generates data (either linear regression or logistic regression)
and creates confidence intervals using 4 techniques:
    - sandwich confidence interval for linear and logistic regression (used as a baseline)
...
This file allows you to vary the following hyperparameters:
    - model type (linear or logistic regression)
    - sample size N
    - dimension D
    - Covariance type ("I", "Toeplitz", "EquiCorr")
    - type 1 error rate (alpha_level) - produces (1-alpha_level)-confidence intervals for theta.
    - ASGD step size hyperparameter c
    - other ASGD hyperparameters as desired, defaults we use area below:
        - alpha_lr = .505
        - burn_in=False
        - burn_in_threshold = 1
        - initializer = True
...
To run one set of simulations, set run_once=True and loop=False, then adjust the
hyperparameters in the 'if run_once==True' code chunk. This will run only S
simulations using one set of hyperparameters.

To run over several hyperparameters at once, set run_once=False and loop=True, then
adjust the hyperparameters in the 'if loop==True' code chunk.

Once the file is run, a file Hulc_sims_long.csv is created and results are 
continuously appended row by row.

To run the full set of simulations, we recommend running multplie parallel
...
if loop==True:
    ...
    cov_type_grid = ["I", "Toeplitz", "EquiCorr"]
