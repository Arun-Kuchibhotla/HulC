...
This file generates linear regression data in multiple dimensions
...
#The SGD gradient (see equation 5 in Overleaf https://www.overleaf.com/project/652d5761fbda66b388df8d72)
...
#This is the SGD Hessian matrix (see equations 6-7 in Overleaf https://www.overleaf.com/project/652d5761fbda66b388df8d72)
...
def SGD_hessian_linear(x_i):
    ...
    #This is matrix A in the Chen paper (see their Example 2.1)
...
def eta_t(t, c = .01, alpha=.501):
    ...
    # WARNING: c in (.01, .02) and alpha in (.51, .6) seems to be the "magic" numbers
...
#Produce CI's according to Chen and Lee
...
def ASGD_plugin_CI(theta_bar, A_bar, S_bar, n, alpha_level = .05):
    ...
    # Chen & Lee 2021, Section 4, but notice they have a typo: denominator must have sqrt(n))
...
def SGD_A_S_logistic(x_i, theta, ytype="neg11"):
    ...
    #Comes from Chen paper (their Example 2.2)
